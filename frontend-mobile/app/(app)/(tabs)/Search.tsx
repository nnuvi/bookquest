...
export default function Search() {
  // const { search } = useGlobalSearchParams<{ search: string }>();
  ...
  console.log("books sesrch: ", books);
  console.log("user sesrch: ", users);
  ...
  return (
    <Screen>
      ...
      <ScrollView className="flex-1">
        {/** Books Search */}
        <AppText className="m-2 font-semibold px-2">Books</AppText>

        {books && (
          <BookList
            data={books}
            onItemPress={(item) => router.push(`/books/BookDetails/${item.bookId}`)}
            refreshing={booksRefetching}
            onRefresh={booksRefetch}
          />
        )}

        {/** User Search */}
        <AppText className="m-2 font-semibold px-2">Users</AppText>
...
//         (user: { _id: any }) => user._id !== authUser?._id,
//       );
